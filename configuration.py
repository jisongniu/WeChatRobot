...
class Config(object):
    ...
    def _load_notion(self):
        """加载 Notion 配置"""
        self.NOTION = {}
        if "NOTION" in self.config:
            self.NOTION = {
                "TOKEN": self.config["NOTION"].get("TOKEN"),
                "LISTS_DB_ID": self.config["NOTION"].get("LISTS_DB_ID"),
                "GROUPS_DB_ID": self.config["NOTION"].get("GROUPS_DB_ID"),
                "ADMINS_DB_ID": self.config["NOTION"].get("ADMINS_DB_ID"),
                "KEYWORDS_DB_ID": self.config["NOTION"].get("KEYWORDS_DB_ID", "1694e93f568280ba9baef81b52ba4afa")  # 添加默认值
            }
